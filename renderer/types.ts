 ...
 *
 * The renderer treats each element uniformly through GooseElement.
 * ------------------------------------------------------------------ */

export type GooseKind =
  | 'button'
  | 'glass-shape'
  | 'plain-rect'
  | 'progressive-blur'
...
export interface GooseElement extends GooseBtnCfg {
  kind: GooseKind
  plainRect?: GooseRectSpec
  progressiveBlur?: GooseBlurSpec
  text?: GooseTextSpec
  ...
  icon?: {
     ...
     *  paths ported from Compose ImageVector (Material icons). */
    viewport?: number
    /** Layout box size in px (default = size). See GooseTextSpec.icon. */
    layoutSize?: number
  }
   ...
   *   - scale = gooseLerp(1, 1.5, pressProgress) (knob grows on press)
  ...
  isToggleKnob?: {
     ...
     * The scale is (gooseLerp(2/3, 0.75, pressProgress), gooseLerp(0, 0.75, pressProgress))
   ...
   *   scale = gooseLerp(1, 1 + 16dp/width, pressProgress)
   ...
   *   scale = gooseLerp(1, 1.2, pressProgress)
   * Also shifts by panelOffset during drag.
   */
  isBottomTabContent?: {
    groupId: string
  ...
  /**
   * Render ON TOP of all other elements (after the main gooseRender loop).
   ...
   *  uBackdrop texture binding in gooseGlassPass — the element
   ...
   *  gooseRenderDlg) holding wallpaper+scrim+colorControls as one opaque
   ...
   *  AFTER gooseRenderBG — before any element (dim, tiles) composites on
   ...
   *  (updated live via gooseGravAngle) instead of the static highlight.angle
   ...
   *  pushed to the renderer via gooseGravAngle, not via React state). */
   ...
   *  card for now. The renderer's gooseLoadCSDF() must have been called
   *  for the element's (w, h, radius) before rendering — see methods-gooseRender.ts. */
  useContinuousSdf?: boolean
}

/* Per-element interaction state — mirrors InteractiveHighlight.kt. */
export interface GooseElemState {
  ...
  // pushed in via gooseInterVal(). The renderer just reads them
...
/* ------------------------------------------------------------------ *
 * GooseTglState — faithful port of DampedDragAnimation.kt.
 *
 * The renderer maintains one GooseTglState per groupId. The fraction
 ...
 * ------------------------------------------------------------------ */
export interface GooseTglState {
  ...
  isDragging: boolean
  // True after a drag release (gooseTglDragEnd/gooseSldDragEnd) — enables
  // velocity tracking from fraction rate-of-change. Set false by
  // gooseTglTarget/gooseTabSel (tap = no velocity animation).
  ...
  // Set when the toggle group is first created (via gooseEnsureTgl).
  pressedScale: number
