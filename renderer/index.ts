...
import { gooseDestroyInnerShadowCache } from './inner-shadow-cache'
import type { GooseInnerShadowCacheEntry } from './inner-shadow-cache'
import type {
  GooseElement,
  GooseElemState,
  GooseTglState,
...
export class LiquidGlassRenderer {
  ...
  dpr = 0 // 0 = not yet set; gooseResize() sets default cap on first call
  buttonConfigs: GooseElement[] = []
  buttonStates = new Map<string, GooseElemState>()
  /** Toggle group state — keyed by groupId. Faithful port of DampedDragAnimation.kt. */
  toggleStates = new Map<string, GooseTglState>()
  scrollY = 0
  scrollVelocity = 0
  contentHeight = 0
  cssWidth = 0
  ...
  innerShadowMaskCache = new Map<string, GooseInnerShadowCacheEntry>()
  ...
  static readonly gooseTabScale = 78 / 56
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.gl = gl
    // Enable derivative functions (fwidth/dFdx/dFdy) for adaptive edge AA.
    ...
    // Stroke masks are created lazily in gooseGlassPost and cached by
  ...
  gooseBlurProg(tapCount: number): void {
  ...
  gooseBlur(srcTex: WebGLTexture, radius: number): WebGLTexture {
    const gl = this.gl
    const w = this.fboW
    const h = this.fboH
  ...
  gooseHLBlurProg(tapCount: number): void {
   ...
   *  Saves/restores the currently-bound framebuffer. */
  gooseHLBlur(srcTex: WebGLTexture, sigmaPx: number): WebGLTexture {
    const gl = this.gl
    const w = this.fboW
    const h = this.fboH
...
// Re-export all public types so callers can `import type { GooseElement, ... } from './renderer'`.
export type {
  GooseRect,
  GooseHL,
  GooseBtnCfg,
