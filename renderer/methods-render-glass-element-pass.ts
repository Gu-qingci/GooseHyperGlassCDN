...
declare module './index' {
  interface LiquidGlassRenderer {
    gooseGlassPass(state: GlassRenderState, curTex: WebGLTexture): void
  }
}
...
export const glassElementPassMethods = {
  ...
  gooseGlassPass(
    ...
    if (el.isToggleKnob) {
      ...
      //       val scaleX = gooseLerp(2f / 3f, 0.75f, progress)
      //       val scaleY = gooseLerp(0f, 0.75f, progress)
      //       scale(scaleX, scaleY) { drawBackdrop() }
      //     }
      //   )
      //
      ...
      if (el.isToggleKnob.trackColorOff && el.isToggleKnob.trackColorOn && el.isToggleKnob.trackW && el.isToggleKnob.trackH) {
        ...
        // Lerp track color: gooseLerp(trackColorOff, trackColorOn, fraction)
    ...
    // and tab content (LocalLiquidBottomTabScale gooseLerp(1, 1.2, progress)).
    if (el.isBottomTabIndicator) {
    ...
    // in gooseRenderGlass won't run (no blur needed), so keep inline blur
    ...
    // 256×256, RGBA, cached by (w, h, radius) — see gooseLoadCSDF().
    ...
    // the 2-pass blur (in gooseRenderDlg + gooseRenderGlass's blur
