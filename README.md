...
martin65536 在其 README 中设置了"耻辱柱"章节，列出了以下指控。由于本项目的渲染引擎代码源自 martin65536/liquid-glass-webgl，这些指控的逻辑我想喷呵呵。

### "系统性抹除署名"

他在 Issue 里说我"在宣传文案中把上游作者抹除"。

看看我现在的官网顶部——**martin65536/liquid-glass-webgl** 几个大字 28px 加粗橙色爆闪彩虹动画，比他自己的 README 标题还大。文档页底部也有。GitHub README 表格专门一行写 WebGL 移植版，链接直达他仓库。

他说我"抹除"他——除非他瞎了，否则怎么可能看不见？

### "强制降分辨率且不可调"

我的代码来自他的仓库。他说我有这个问题——那他的原版也有这个问题。我加了 dpr 属性让用户可调，他的版本让用户去设置页手动调。同一件事，两套说法。

### "滥用 blur 滤镜"

Blur 参数直接对应 Kyant 原版 Android 项目的玻璃模糊参数。如果他认为这是"滥用"，那他从 Kyant 移植的时候就已经"滥用"了。

### "未处理浏览器默认点击行为"

呵呵，这个吗，但凡经历过九年义务教育的都知道，那是使用者的事，管我屁事？
...
还有，再说一遍，我的驱动器是他的，那就说明他也有毛病。

### "连 G2 连续曲率圆角都未能正确实现"

G2 continuous curvature 是公开数学方案，不是 martin65536 的独占技术。而且——他的代码里 G2 曲率实现是从 Kyant 的 Kotlin 源码移植的，我的代码是从的他仓库拉的。如果他能写对，我自然也能写对。如果我写错了，那说明他的原版也有问题。这货一边说"你的代码和我高度一致"，一边又说"你这里写错了但我那里是对的"...
- 哎呀，指出这么多错误代码还能高度一致也是个人物。

### "开源了又不让用"

他的仓库用的是 **Apache-2.0 许可证**。Apache-2.0 明确允许"复制、修改、分发"——这是开源许可证的基本功能。他选了 Apache-2.0，就意味着他同意了别人可以用他的代码。

现在我用了，你跑来骂我"抄袭"。那我问你：**你开源干屁？**
...
- 喷不动了，看这里[接着喷](https://blog.goose.cc.cd/post/why-github-is-getting-worse)
