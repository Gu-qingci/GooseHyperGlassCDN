/**
 * SIRI WAVE fragment shader — Apple Siri 声波复刻。
 *
 * 来源：aaaa-zhen/siri-glsl（MIT）siri-wave.html 的 fs-wave（siriWaveCore IR 还原）
 * 移植改动：
 *   - `iTime` 乘 uniform `uSpeed`（组件 speed 属性）
 *   - `WAVE_SCALE` 乘 uniform `uScale`（组件 scale 属性）
 *   - 输出 alpha = 线条亮度：背景透明，便于叠在组件壁纸/渐变上
 *   - 其余常量与算法原样保留
 */
export const SIRI_WAVE_FRAGMENT_SHADER = /* glsl */ `
